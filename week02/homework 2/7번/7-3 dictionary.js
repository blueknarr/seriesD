...
result = []
for(let i of bikes){
    if(i['parkingBikeTotCnt'] <=5)
        result.push(i['stationName'])
}
console.log(result)

function filterStation(bikeCnt){
    let res = []
    for(let i of bikes)
        if(i['parkingBikeTotCnt'] <= bikeCnt)
            res.push(i['stationName'])
    return res
}

console.log(filterStation(3))